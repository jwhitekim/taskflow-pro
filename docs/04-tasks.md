...
| 단계 | 검증 방법 | 완료 |
|---|---|---|
| 1. frontend/ 폴더 및 index.html, app.js 생성 | 2개 파일만 존재 확인 | [ ] |
| 2. Tailwind CDN + Mac OS 톤 레이아웃 구성 | 둥근 모서리/그림자/반투명 카드 확인 | [ ] |
| 3. 추가 폼 화면 구현 | title/due_at/status 입력 후 POST 동작 확인 | [ ] |
| 4. 목록 카드 화면 구현 | status 배지 + 남은 시간 표시 확인 | [ ] |
| 5. 수정 모달 구현 | 카드 클릭 → 모달 → PUT 동작 확인 | [ ] |
| 6. 삭제 기능 구현 | 휴지통 → 확인 → DELETE 동작 확인 | [ ] |
| 7. 테마 토글 + 반응형(360px) 적용 | localStorage 저장 확인, 360px에서 미깨짐 확인 | [ ] |
| 8. 전체 동작 확인 후 git push | 새로고침 데이터 유지 + CRUD 4종 동작 확인 후 push | [ ] |
